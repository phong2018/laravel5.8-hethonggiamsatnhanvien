...
<div class="row justify-content-center">
   <div class="page-header">
		...
		<table class="table panel panel-default">
			<thead>
				<tr>
					<th scope="col">#</th>
					<th scope="col">Tên</th>
					<th scope="col">Màu sắc đại diện</th>
          ...
          <th scope="col">Trạng Thái</th>
			...
			<tbody>
				<?php $i = 1; ?>
				@foreach ($tinhtrangxuly as $ttrang)
				<tr>
					<td>{{ $i }}</td>
					<td>{{$ttrang->tinhtrangxuly_name}}</td>
					<td><i class="fa fa-flag"  style="color:{{$ttrang->tinhtrangxuly_color}};font-size:30px;"</i></td>
          ...
          <td>{{$ttrang->sort_order}}</td>
              ...
              <?php }?></td>
					...
					<a data-original-title="Sửa Chức Vụ" data-toggle="tooltip" class=' btn btn-success' href="{{Route('tinhtrangxuly.edit',['tinhtrangxuly' => $ttrang->tinhtrangxuly_id])}}?token={{session('token')}}"><i class="fas fa-edit"></i></a></td><td>&nbsp&nbsp </td><td>
					<span data-original-title="Xóa Chức vụ" data-toggle="tooltip" class=' btn btn-danger' onclick="delObj('{{Route('tinhtrangxuly.destroy',['tinhtrangxuly' => $ttrang->tinhtrangxuly_id])}}?token={{session('token')}}')" ><i class="fa fa-trash-o " ></i></span></td></table>
	...
	{{ $tinhtrangxuly->appends($data['addurl'])->render() }}
