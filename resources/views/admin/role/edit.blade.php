            ...
            <div class="card-body">
                <form  action="{{$data['action_update']}}"   accept-charset="UTF-8" method="POST" enctype="multipart/form-data">    
                    ...
                    <div class="form-group row">
                        <label for="role_name" class="col-md-2 col-form-label text-md-right">{{ __('Tên Vai Trò') }}</label>

                        <div class="col-md-10">
                            <input id="role_name" type="text" class="form-control{{ $errors->has('role_name') ? ' is-invalid' : '' }}" name="role_name" required value="{{$role->role_name}}" >
                    ...
                    <div class='plgscrollbar'>
                       <table class='borderchinone'>
                        <tr class='headertb'><td>Tên Menu</td><td>Ghi Chú</td><td><input id='checkall' type="checkbox" /></td></tr> 
                        @if(count($menu) > 0)
                        @foreach($menu as $val)
                        <tr><td>{{$val->menu_name}}</td><td>{{$val->menu_note}}</td> <td>
                        <input class='cbmenu' type="checkbox" name="menus[]" value='{{$val->ID_Menu}}'
                        @if(in_array($val->ID_Menu, $menu_role))
                            checked
                        @endif
